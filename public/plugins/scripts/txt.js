...
$(document).ready(function () {
    ...
    $(document).on("click", "#approveBtn", function () {
        ...
        <div class="d-flex justify-content-between font-weight-bold border-top pt-2 mt-2 text-primary">
    ...
    $("#approveModal").on("shown.bs.modal", function () {
        ...
        const previousRemaining =
            parseFloat($("#previousRemaining").val()) || 0;

        let defaultGiven;
        defaultGiven =
            previousRemaining === 0
                ? window.totalRecoveryAmount
                : previousRemaining;

        $("#givenAmount").val(defaultGiven);
        let newRemaining;
        if (previousRemaining === 0) {
            newRemaining = netTotal - defaultGiven;
        } else {
            newRemaining = previousRemaining - defaultGiven;
        }
    ...
    $(document).on("input", "#givenAmount", function () {
        ...
        if (previousRemaining === 0) {
            newRemaining = netTotal - givenAmount;
        } else {
            newRemaining = previousRemaining - givenAmount;
        }
    ...
    $("#approveForm").on("submit", function (e) {
        ...
        if (
            givenAmount < 0 ||
            remainingAmount < 0 ||
            givenAmount + remainingAmount > netTotal
        ) {
            toastr.error("Invalid amounts entered.", "Error!");
            return;
        }
        ...
        formData.append("previous_remaining", $("#previousRemaining").val());
    ...
    $(document).on("click", "#approveAllBtn", function () {
        Swal.fire({
            title: "Are you sure?",
            text: "You are approving recoveries for ALL recovery men. This action cannot be undone!",
            icon: "warning",
